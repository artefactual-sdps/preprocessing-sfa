...
- Analysis is intentionally reduced to: `Neu` -> `InAnalyse` -> `Analysiert`.
- `POST /api/ImportTasks/{id}/importRuns` creates in-memory run IDs like
  `run-000001`.
- Import is intentionally reduced to: `WirdImportiert` -> `Importiert`.
...
- The mock accepts request hints in uploaded filenames to force per-request
  ...
  exactly as shown:
  - `mock-alleneu`, `mock-allegleich`, `mock-konflikte`, `mock-fehler`
  - `mock-import-erfolgreich`, `mock-import-fehler`
