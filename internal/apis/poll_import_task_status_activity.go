...
func (a *PollImportTaskStatusActivity) Execute(
	...
	for {
		...
		case <-ctx.Done():
			return nil, ctx.Err()
		case <-ticker.C:
			status, err := a.pollStatus(ctx, params.TaskID)
			if err != nil {
				return nil, err
			}
...
func (a *PollImportTaskStatusActivity) pollStatus(
	ctx context.Context,
	taskID string,
) (*gen.ImportTaskStatusResponse, error) {
	res, err := a.client.APIImportTasksIDStatusGet(ctx, gen.APIImportTasksIDStatusGetParams{ID: taskID})
	if err != nil {
