...
`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			act := activities.NewAddPREMISValidationEvent(mockVdr)
			act.SetClock(clockwork.NewFakeClockAt(time.Date(2025, 6, 6, 9, 57, 16, 0, time.UTC)))
			act.SetRng(pseudorand.New(pseudorand.NewSource(1))) // #nosec G404
